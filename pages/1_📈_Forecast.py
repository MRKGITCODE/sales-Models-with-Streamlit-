...
with header:
    header.title(
        "Sales Prediction App With Linear Regression Across Favorita Stores")
...
with st.expander("Make a prediction", expanded=True):
    ...
    holiday = st.selectbox(label="Select a category of holiday", options=[
                           'Holiday', 'Not Holiday', 'WorkDay', 'Additional', 'Event', 'Transfer', 'Bridge'])
    locale = st.radio(label="Select a holiday type", options=[
                      "National", "Regional", "Local", "Not Holiday"], horizontal=True)
    transferred = st.radio(label="Select whether the holiday was transferred or not", options=[
                           "True", "False"], horizontal=True)
    onpromotion = st.number_input(
        label="Please enter the total number of expected items to be on promotions")
...
# Display prediction message inside the expander with HTML formatting
        st.write(
            f"Your total predicted sales will be {formatted_output}", unsafe_allow_html=True)
